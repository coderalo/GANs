...
class CycleGAN:
    ...
    def train(self, config):
        ...
        self.GA2B_optimizer = tf.train.AdamOptimizer(config.g_learning_rate, beta1=config.beta1) \
                .minimize(self.GA_loss, var_list=self.ga2b_vars)
        self.GB2A_optimizer = tf.train.AdamOptimizer(config.g_learning_rate, beta1=config.beta1) \
                .minimize(self.GB_loss, var_list=self.gb2a_vars)
    ...
    def build_model(self):
        ...
        self.real_A = tf.placeholder(tf.float32, [None, self.input_height, self.input_width, self.channels], name="A_real")
        self.real_B = tf.placeholder(tf.float32, [None, self.input_height, self.input_width, self.channels], name="B_real")

        self.fake_A = self.generator(self.B_real, reuse=False, name="generatorB2A")
        self.fake_B = self.generator(self.A_real, reuse=False, name="generatorA2B")
        self.cycle_A = self.generator(self.B_fake, reuse=True, name="generatorB2A")
        self.cycle_B = self.generator(self.A_fake, reuse=True, name="generatorA2B")
        ...
        self.sample_A = tf.placeholder(tf.float32, [None, self.input_height, self.input_width, self.channels], name="A_sample")
        self.sample_B = tf.placeholder(tf.float32, [None, self.input_height, self.input_width, self.channels], name="B_sample")
        ...
        self.DA_fake_sum = tf.summary.scalar("DA_fake_loss", self.DA_fake_loss)
        self.DA_sum = tf.summary.scalar("DA_loss", self.DA_loss)
        self.DB_real_sum = tf.summary.scalar("DB_real_loss", self.DB_real_loss)
        self.DB_fake_sum = tf.summary.scalar("DB_fake_loss", self.DB_fake_loss)
    ...
    def generator(self, input_tensor, reuse=False, name="generator"):
        with tf.variable_scope(name) as scope:
            ...
            c2 = tf.nn.relu(instance_norm(conv2d(c1, self.gf_dim*2, 3, 2, name='g_e2_conv'), 'g_e2_in'))
            c3 = tf.nn.relu(instance_norm(conv2d(c2, self.gf_dim*4, 3, 2, name='g_e3_conv'), 'g_e3_in'))
            ...
            d1 = deconv2d(r9, self.fd_dim*2, 3, 2, name='g_d1_dc')
            d1 = tf.nn.relu(instance_norm(d1, 'g_d1_bn'))
            d2 = deconv2d(d1, self.fd_dim, 3, 2, name='g_d2_dc')
    ...
    def train_batch(self, counter):
        ...
        errGA2B = self.G_loss.eval({self.real_B: batch_A, self.real_B: batch_B})
        errGB2A = self.G_loss.eval({self.real_A: batch_A, self.real_B: batch_B})
    ...
    def prepare_data(self):
        self.images_A = glob.glob(os.path.join(self.data_dir, "A/*.jpg"))
        self.images_B = glob.glob(os.path.join(self.data_dir, "B/*.jpg"))
